 ...
 * }
 */
class OrbifoldPathTracer : public MonteCarloIntegrator {
public:
    OrbifoldPathTracer(const Properties &props)
    ...
    OrbifoldPathTracer(Stream *stream, InstanceManager *manager)
    ...
    bool preprocess(const Scene *scene, RenderQueue *queue, const RenderJob *job, int sceneResID, int sensorResID, int samplerResID) {
        bool ret = MonteCarloIntegrator::preprocess(scene, queue, job, sceneResID, sensorResID, samplerResID);
        if (ret) {
            return m_odata.setType(OrbifoldData::OrbifoldTypeFromString(m_otype));
        }
        return ret;
    }
    ...
    std::string m_otype;
    OrbifoldData m_odata;

	MTS_DECLARE_CLASS()
};

MTS_IMPLEMENT_CLASS_S(OrbifoldPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(OrbifoldPathTracer, "Orbifold MI path tracer")
